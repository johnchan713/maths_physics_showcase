...
def inherit_history(joined, rows, manifest, grid, start_time):
    """Use the immediate parent's restart metadata and the full earlier budgets."""
    if not rows or rows[-1]["time"] != start_time or manifest["settings"]["grid"] != grid:
        raise ValueError("Wrong source trajectory")
    if joined["initial_state_sha256"] != manifest["initial_state_sha256"]:
        raise ValueError("Joined initial identity differs from the immediate parent")
    times = [budget["time"] for budget in joined["budgets"]]
    if not times or times[-1] != start_time or any(b <= a for a, b in zip(times, times[1:])):
        raise ValueError("Earlier critical budget clock is incomplete or duplicated")
    if [b for b in joined["budgets"] if b["time"] > manifest["settings"]["start_time"]] != manifest["budgets"]:
        raise ValueError("Joined budgets differ from the immediate parent")
    return {**manifest, "budgets": joined["budgets"]}, rows


def source_history(protocol, source, grid):
    """Run the archived reader in isolation so same-named modules cannot mix."""
    specification = protocol["source_history_reader"]
    reader = RESEARCH / specification["path"]
    assessment = reader.parent / "assessment.json"
    if reader.parent != source.parent or driver.sha(reader) != specification["sha256"]:
        raise ValueError("Earlier history reader changed")
    if driver.sha(assessment) != specification["assessment_sha256"]:
        raise ValueError("Earlier assessment changed")
    if json.loads(assessment.read_text())["status"] != "passed-preliminary-finite-amplification-continuation-gates":
        raise ValueError("Earlier trajectory did not pass its declared gates")
    code = ("import json, sys; sys.path.insert(0, sys.argv[1]); "
            "from analyze import history; print(json.dumps(history(int(sys.argv[2])), allow_nan=False))")
    result = subprocess.run([sys.executable, "-c", code, str(reader.parent), str(grid)],
                            check=True, capture_output=True, text=True)
    joined, rows, manifest = json.loads(result.stdout)
    return inherit_history(joined, rows, manifest, grid, protocol["start_time"])


def provenance(grid, solver=None):
    protocol = json.loads((ROOT / "protocol.json").read_text())
    source = RESEARCH / protocol["source_histories"][str(grid)]
    manifest, rows = source_history(protocol, source, grid)
...
def main():
    ...
    if restart_budget["checkpoint_sha256"] != original["uncompressed_sha256"]:
        raise ValueError("Restored field differs from the verified .12 state")
    manifest = {
